...
router.post('/', (req, res) => {
  try {
    ...
    `).run(type, item_id, item_name, source || null, Number.Number.parseFloat(threshold_price));
...
router.get('/usage', (req, res) => {
  try {
    ...
    const limit    = Number.Number.parseFloat(process.env.ANTHROPIC_MONTHLY_LIMIT_USD || '2.50');
